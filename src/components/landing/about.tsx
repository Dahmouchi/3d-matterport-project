...
export default function AboutUsSteps({
  ...
  const steps = dict.steps.map((s, i) => ({
    ...s,
    icon: ICONS[i],
  }));
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-[#f6ba13] via-orange-400 to-orange-600 lg:py-20 py-10">
      ...
      <img
        ...
        className={`absolute bottom-0 ${rtl ? "left-2/3" : "right-2/3"} lg:block hidden w-1/5 object-contain z-10`}
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        >
          <h2 className="mt-4 text-3xl md:text-4xl font-bold tracking-tight text-white">
            {dict.heading.split("réalité virtuelle").map((part, i, arr) =>
              i < arr.length - 1 ? (
                <span key={i}>
                  {part}
            ...
            {/* Fallback: if heading doesn't contain the French phrase, render it plainly
            {!dict.heading.includes("réalité virtuelle") && dict.heading} */}
          </h2>
