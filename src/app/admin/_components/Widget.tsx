...
export default function Widget({ widgetData }: WidgetProps) {
  ...
  return (
    <Card className="w-full h-full aspect-[2] relative overflow-hidden rounded-none border-2 border-zinc-800 bg-zinc-900/50 text-white">
      <TVNoise opacity={0.3} intensity={0.2} speed={40} />
      <CardContent className="flex-1 flex flex-col justify-between text-xs font-medium uppercase relative z-20">
        <div className="flex justify-between items-center">
          <span className="opacity-50">{dateInfo.dayOfWeek}</span>
          <span>{dateInfo.restOfDate}</span>
        </div>
        <div className="text-center">
          <div className="text-3xl font-display" suppressHydrationWarning>
